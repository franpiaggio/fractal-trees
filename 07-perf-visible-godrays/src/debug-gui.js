...
export function buildDebugGui({ post, env, grass, scene, preset, onReturnToMenu }) {
  const gui = new GUI({ title: `07 · ${preset.label}` });
  ...
  if (post.godrays) {
    ...
    const p = {
      density:             preset.godraysDensity ?? 0.013,
    ...
    fGod.add(p, 'density', 0, 0.05, 0.0005).onChange(apply);
  ...
  gui.add({ log: () => console.log('[07] current GUI state — read controllers above') }, 'log')
    .name('log to console');
