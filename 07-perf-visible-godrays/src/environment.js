...
// Cool haze on purpose: warm cream godrays read most strongly against a
// slightly cool background, the way real forest light-shafts photograph.
const SKY = new THREE.Color(0xc3d6ef);
// v06's sun sat at ~43° elevation — almost overhead. The auto-explorer keeps
// the camera level with the horizon, so you were never looking *toward* the
// light, and godray shafts are strongly forward-scattering: invisible unless
// the sun is roughly ahead and low. Dropped to ~21° so the shafts rake
// horizontally between the trunks where the camera actually looks.
const SUN_DIR = new THREE.Vector3(0.55, 0.27, 0.42).normalize();
