...
import { pixelUniforms } from './pixel.js';
...
export function buildDebugGui({ post, env, grass, scene, world, preset, grControl, onReturnToMenu }) {
  const gui = new GUI({ title: `11 · ${preset.label}` });

  // ── Pixel-art look ───────────────────────────────────────────────────────────────
  if (post.effects?.pixel) {
    const fPixel = gui.addFolder('Pixel');
    const p = {
      wobble: pixelUniforms.uSnap.value,
      levels: post.effects.pixel.levels,
      dither: post.effects.pixel.dither,
    };
    // lower snap = chunkier wobble; invert the slider so right = more wobble
    fPixel.add(p, 'wobble', 20, 400, 5).name('vertex grid').onChange((v) => { pixelUniforms.uSnap.value = v; });
    fPixel.add(p, 'levels', 4, 64, 1).name('colour levels').onChange((v) => { post.effects.pixel.levels = v; });
    fPixel.add(p, 'dither', 0, 2, 0.05).name('dither').onChange((v) => { post.effects.pixel.dither = v; });
    fPixel.open();
  }
  ...
  gui.add({ log: () => console.log('[11] current GUI state — read controllers above') }, 'log')
    .name('log to console');
