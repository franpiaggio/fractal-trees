...
import { PixelEffect } from './pixel.js';
...
export function buildPipeline(renderer, scene, camera, preset, light) {
  ...
  // v11: PS1 look. No SMAA (we want hard pixels), no DoF (PS1 was sharp). The
  // PixelEffect quantises colour + ordered-dithers, last in the chain so it acts
  // on the final image (and at the low internal resolution).
  const pixel = new PixelEffect({ levels: 22, dither: 1.0 });

  const effects = [bloom, toneMapping, hueSat, brightnessContrast, vignette, pixel]
    .filter(Boolean);
  ...
  function resize() {
    // Size the composer to the renderer's DRAWING BUFFER (the small PS1 buffer),
    // not the canvas CSS size — otherwise the composer would render full-res.
  ...
  return {
    ...
    effects: { bloom, dof, toneMapping, hueSat, brightnessContrast, vignette, pixel },
  };
}
