...
export function buildWorld(scene, templates, { worldSeed = 1337, viewChunks = 4, treeCastShadow = false, renderDistance = 28 } = {}) {
  ...
  function update(camera) {
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      // Distance cull FIRST — a cheap squared-distance test rejects the ~95% of
      // candidates that fog hides, before the costlier frustum/sphere math. Rare
      // giants tower over the canopy, so they're allowed to render from farther
      // (renderDistance + their height) — you see them looming before you arrive.
      const dx = t.x - camX, dz = t.z - camZ;
      const reach = t.giant ? renderDistance + h * 0.7 : renderDistance;
