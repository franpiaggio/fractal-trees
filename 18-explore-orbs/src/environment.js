...
export function buildEnvironment(scene, renderer, preset) {
  ...
  // Re-aim the sun. Mutates the shared SUN_DIR in place so every live reader
  // (updateSun's light offset, godray falloff, etc.) follows; also updates the sky
  // dome's glow direction (which holds an independent clone).
  function setSunDir(dir) {
    SUN_DIR.copy(dir).normalize();
    skyMat.uniforms.uSunDir.value.copy(SUN_DIR);
  }

  return { sun, hemi, ground, skydome, updateSun, setPalette, setTime, setCloud, setSunDir, skyColor: SKY, sunDir: SUN_DIR };
}
