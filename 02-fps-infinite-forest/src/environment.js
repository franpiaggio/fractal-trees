...
// Fog tuned to *completely hide the grass-patch boundary*. The patch fade
// band sits at 31.5–35.8 m; we need that range to be in heavy fog so the
// player can never see blades scaling in/out as they advance. With NEAR=8,
// FAR=40:
//   8  m  → fog just begins
//   25 m → 53 % opacity (light haze; trees blur)
//   32 m → 75 % opacity (patch fade starts — invisible)
//   36 m → 88 % opacity (patch outer edge — invisible)
//   40 m → 100 % (fully fogged out)
const FOG_NEAR = 8;
const FOG_FAR = 40;
