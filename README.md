...
| **13** | `13-terrain/`               | Fork of 12 with rolling **terrain relief** (shared height field in JS + GLSL, so ground/trees/grass/player all follow the hills), **grass colour variation** within the green gamut, a real **snow** ground texture in winter, a smooth GSAP **Demo** mode (renamed from auto-explore, flies toward the sun so the godrays stay in frame), **Exit returns to the menu without reloading**, **ambient audio + a music track**, a **safe** in-game Randomize (R key) that also varies the post look + logs the config, and a fully **English** UI. Port 5185. | Vite + vanilla JS + Three.js |
| **14** | `14-infinite-forest/`       | Refactor of 13 into a **two-screen** app sharing one engine (`src/scene.js`): **Explore** (live scene, no recording) and **Record** (capture settings as a UI form → downloads a `.webm`), cross-linked. Adds **music autoplay**, audio buttons in the desktop top bar, mobile **Randomize** bottom-centre, hidden stats HUD, and **persistent leaf-canopy anchors** (autumn leaves finish falling). Also a standalone repo deployed to GitHub Pages. Port 5186. | Vite + vanilla JS + Three.js |
| **15** | `15-ps1/`                   | Fork of 13 chasing a **tasteful PS1 vibe**: resolution-tied **vertex jitter** (no sub-pixel precision), low-res internal buffer upscaled nearest, **nearest+mipmapped** textures, **15-bit colour + 4×4 Bayer dither**, and the modern post (godrays/bloom/DoF/SMAA) stripped. A live **PS1** GUI folder dials internal resolution, jitter, colour levels and dither. Bushes/leaves opt out of snapping to avoid flicker. Port 5187. | Vite + vanilla JS + Three.js |
| **16** | `16-ps1/`                   | Fork of 15 to keep **exploring the PS1 look** (same toolkit). Scratchpad for affine warp, banding, palette and fog ideas. Audio off by default. Port 5188. | Vite + vanilla JS + Three.js |
