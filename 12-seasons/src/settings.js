...
const SEASON_LIST = [
  ['otono', 'Otoño'], ['invierno', 'Invierno'], ['primavera', 'Primavera'], ['verano', 'Verano'],
];
...
export function buildSettings({ onReseed, onSeason } = {}) {
  ...
  // ── Season ──
  card.appendChild(groupTitle('Estación'));
  ...
  randomizers.push(slider(card, { label: 'Claros (mínimo)', min: 0, max: 20, step: 1, value: CONFIG.densityMin, onInput: (v) => setForestConfig({ densityMin: v }) }));
  randomizers.push(slider(card, { label: 'Espesura (máximo)', min: 5, max: 40, step: 1, value: CONFIG.densityMax, onInput: (v) => setForestConfig({ densityMax: v }) }));
  randomizers.push(slider(card, { label: 'Tamaño de zona (m)', min: 30, max: 250, step: 5, value: CONFIG.densityZone, onInput: (v) => setForestConfig({ densityZone: v }) }));

  // ── Groves ──
  card.appendChild(groupTitle('Rodales (groves)'));
  randomizers.push(slider(card, { label: 'Tamaño de rodal (m)', min: 20, max: 200, step: 5, value: CONFIG.groveZone, onInput: (v) => setForestConfig({ groveZone: v }) }));
  randomizers.push(slider(card, { label: 'Mezcla en bordes', min: 0, max: 0.6, step: 0.01, value: CONFIG.offSpecies, fmt: (v) => v.toFixed(2), onInput: (v) => setForestConfig({ offSpecies: v }) }));
  randomizers.push(slider(card, { label: 'Cantidad de arbustos', min: 0, max: 0.6, step: 0.01, value: CONFIG.bushChance, fmt: (v) => v.toFixed(2), onInput: (v) => setForestConfig({ bushChance: v }) }));

  // ── Giants ──
  card.appendChild(groupTitle('Árboles gigantes (raros)'));
  randomizers.push(slider(card, { label: 'Frecuencia', min: 0, max: 0.1, step: 0.005, value: CONFIG.giantChance, fmt: (v) => v.toFixed(3), onInput: (v) => setForestConfig({ giantChance: v }) }));
  ...
  const randomBtn = mkBtn('🎲 Randomizar', false);
  randomBtn.addEventListener('click', randomizeAll);
  const reseedBtn = mkBtn('↻ Semilla', false);
