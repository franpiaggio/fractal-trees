...
function boot(preset, mode) {
  ...
  if (!MOBILE) {
    const gui = buildDebugGui({ post, env, grass, scene, world, preset, grControl, onReturnToMenu: returnToMenu });

    if (gui?.domElement) gui.domElement.classList.add('dbg-gui');

    // "Ocultar GUI" hides ALL on-screen UI (these buttons, the lil-gui panel and
    // the stats HUD) for a clean render-only view, leaving just a small restore
    ...
    const placeBtn = (text, leftPx, onClick) => {
      const b = makeHudBtn(text, onClick);
      b.classList.add('ui-toggleable');
      b.style.cssText += `position:fixed;top:12px;left:${leftPx}px;z-index:2147483647;`;
      document.body.appendChild(b);
      return b;
    };
    placeBtn('Salir', 12, returnToMenu);
    placeBtn('Randomizar', 78, randomizeForest);
    placeBtn('Ocultar GUI', 196, () => document.body.classList.add('ui-hidden'));
    ...
    window.addEventListener('keydown', (e) => { if (e.code === 'KeyO') document.body.classList.toggle('ui-hidden'); });
  }
