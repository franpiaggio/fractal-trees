...
export function buildGrass(scene, options = {}) {
  ...
  // uniform, so it lights up the moment the alpha arrives).
  const loader = new THREE.TextureLoader();
  loader.load(ALPHA_URL, (tex) => {
    tex.colorSpace = THREE.LinearSRGBColorSpace;
    tex.wrapS = tex.wrapT = THREE.ClampToEdgeWrapping;
    uniforms.uGrassAlpha.value = tex;
  });
  ...
  return { mesh, material: mat, uniforms, update, setTime, total, halfSize: opts.halfSize };
}
