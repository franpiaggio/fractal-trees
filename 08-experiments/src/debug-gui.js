...
export function buildDebugGui({ post, env, grass, scene, preset, grControl, onReturnToMenu }) {
  const gui = new GUI({ title: `08 · ${preset.label}` });
  ...
  gui.add({ log: () => console.log('[08] current GUI state — read controllers above') }, 'log')
    .name('log to console');
