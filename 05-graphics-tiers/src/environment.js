...
// Procedural ground texture: multi-octave value noise blending three greens
// with occasional dirt patches. Built once on boot, tiled across the plane.
...
export function buildEnvironment(scene, renderer, preset) {
  ...
  // Tier-driven shadows. shadowMapSize === 0 ⇒ no shadow pass at all.
  // Trees are kept out of the shadow pass anyway (castShadow=false in
  // world.js), so the cost is mostly the ground self-shadow. Low tier
  ...
  // Only receive shadows if shadows are on.
  ground.receiveShadow = preset.shadowMapSize > 0;
  scene.add(ground);
